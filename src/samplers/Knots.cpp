...
//(_knots_Node->parents()[x]->value(chain))
/*
std::vector<double const*> const &Knots::parameters(unsigned int chain, StochasticNode const * _knots_Node) const
{


    for (unsigned int i = 0; i < parameter.size(); ++i) {
        *_parameters.push_back(parameter[i]->value(chain));
    }
...
double Knots::acceptProbBalance(unsigned int const &chain, TypeOfUpdate type) const
{
  ...
  //std::cout << parameters(chain, _knotsNode) << std::endl;
  //std::cout << "test" << std::endl;

  std::vector<double const*> _parameters;
  std::vector<Node const *> const &parameter = (_knotsNode->parents());
    _parameters.reserve(parameter.size());
	for (unsigned int i = 0; i < parameter.size(); ++i) {
	    _parameters.push_back(parameter[i]->value(chain));
	}
