...
void RJumpSpline::calBeta(double *betas, bool const &current, unsigned int const &chain)
{
  ...
  */
  ...
  std::vector<StochasticNode *> const &stoch_children =
...
void RJumpSpline::calPost(bool const &current, unsigned int chain)
{
  ...
  vector<StochasticNode *> const &stoch_children = _mgv->stochasticChildren();
  ...
  */
  ...
  //for (int i=0; i<nrow; ++i)
    //std::cout << *(Acopy + nrow * i + i) << std::endl;
/*
  for (int i=0; i<nrow; ++i)
...
//newly added function, because JAGS 4.3.0 stochastic node does not have parameters() function
/*
std::vector<double const*> const &RJumpSpline::parameters(unsigned int chain, StochasticNode const * _knots_Node) const
{

    std::vector<std::vector<double const*> > _parameters;
    std::vector<Node const *> const &parameter = (_knots_Node->parents());
    //std::cout << parameter.size() << std::endl;
    std::cout << parameter.size() << " " << sizeof(parameter) << std::endl;
    for (unsigned int n = 0; n < _knots_Node->nchain(); ++n) {
            std::cout << "begin1" << n << std::endl;
	_parameters[n].reserve(parameter.size());
            std::cout << "begin2" << n << std::endl;
	for (unsigned int i = 0; i < parameter.size(); ++i) {
            std::cout << "begin para" << i << std::endl;
	    _parameters[n].push_back(parameter[i]->value(n));
    std::cout << (parameter[i]->value(n)) << std::endl;
    std::cout << "end para" << i << std::endl;
	}
...
*/
double const*RJumpSpline::parameters(unsigned int chain, StochasticNode const * _knots_Node, unsigned int x)
{

    return (_knots_Node->parents()[x]->value(chain));
}
    ...
    Sampler(gv),_mgv(gv)
{
    ...
    {
        //std::cout << (_snode->parents()[0] << std::endl;
        //std::cout << *(_snode->parents()[0]->value(0) + 1) << std::endl;

    PriorForT[0] = parameters(0,_snode,5)[0 + 2 * i];
    //std::cout << PriorForT[0] << std::endl;
	//PriorForT[0] = parameters(0, _snode)[5][0 + 2 * i];
	PriorForT[1] = parameters(0, _snode,5)[1 + 2 * i];
	//std::cout << PriorForT[1] << std::endl;

	//KLimits[0] = 0;
	//KLimits[1] = static_cast<unsigned int>(_snode->parameters(0)[1][i]);

	TLimits[0] = parameters(0, _snode,3)[i];
	TLimits[1] = parameters(0, _snode,4)[i];
    ...
    _tauOf3rdColumn = parameters(0, _snode,6)[0];
    _tauOf4thColumn = parameters(0, _snode,8)[0];

    //Prior precision for first value in the 3rd column NON-STOCHASTIC
    _tauOfFirstIn3rdColumn= parameters(0, _snode,7)[0];

    //autoregressive coefficient
    _rho =  parameters(0, _snode,9)[0];
    _etaRho =  parameters(0, _snode,10)[0];
...
double RJumpSpline::llZ(bool const &current, unsigned int chain) const
{
  ...
  vector<StochasticNode *> const & children = _mgv->stochasticChildren();
  ...
  for(unsigned int c = 0; c < _nchain; ++c)
    {
      ...
      //Prior precision for 3rd and 4th column error terms;
      _tauOf3rdColumn = parameters(c, _snode,6)[0];
      _tauOf4thColumn = parameters(c, _snode,8)[0];

      //autoregressive coefficient
      _rho =  parameters(c, _snode,9)[0];
      _etaRho = parameters(c, _snode,10)[0];
      //std::cout << _etaRho << std::endl;
